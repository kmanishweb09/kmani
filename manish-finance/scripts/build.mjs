...
async function buildServer(archive, scriptHash, version) {
  ...
  const r = await esbuild.build({
    ...
    plugins: [virtualModules({ "virtual:finance-archive": archive, "virtual:finance-build-info": { version, pageScriptHash: scriptHash, archiveVersion: archive.version } })],
    logLevel: "warning",
  });
  const code = readFileSync(outfile, "utf8");
  const forbidden = [/from\s+["']node:/, /require\(["'](fs|path|child_process|os|net)["']\)/, /\bprocess\.env\b/, /__dirname/];
  for (const re of forbidden) {
    if (re.test(code)) throw new Error(`finance.mjs contains a Node-only construct matching ${re}`);
  }
  return r.metafile;
}
